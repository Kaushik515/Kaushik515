## 👋 Hi, I'm Kaushik Kotha

Software Engineer II at HCL Software | Full Stack Developer 

I work on enterprise-grade backend systems contributing to HCL SX, an AI-driven service management product.  
My focus is on building scalable REST APIs, designing efficient data models, and developing microservices-based architectures.

---

### 🚀 What I Do

- Develop and maintain backend microservices in Node.js
- Design and deploy scalable REST APIs
- Work extensively with MongoDB for optimized data modeling
- Build standalone services including automated table API systems
- Contribute to full-stack feature development in enterprise products

---

### 🛠 Tech Stack

**Backend**: Node.js, Express.js
**Database**: MongoDB
**Frontend**: React.js, JavaScript, HTML, CSS
**Languages**: C++, JavaScript
**Tools & Platforms**: Git, Postman, Bruno, VS Code, DBeaver
**Core CS**: Data Structures, DBMS, Operating Systems, Computer Networks 

---

### 🔥 Featured Projects

#### 🚗 ParkEasy – Full Stack Parking Management System  
[Client Repository](https://github.com/Kaushik515/park-easy-client) | 
[Server Repository](https://github.com/Kaushik515/park-easy-server) | 
[Live Project](https://park-easy-client.onrender.com)

- MERN-based parking space booking and management system  
- Implemented secure JWT-based authentication  
- Designed scalable MongoDB schemas  
- Built RESTful APIs with proper route and controller separation  

---

#### 🏨 Hotel Booking System  
[Client Repository](https://github.com/Kaushik515/Hotel-Management-FrontEnd) | 
[Server Repository](https://github.com/Kaushik515/Hotel-Management) | 
[Live Project](https://hotel-management-frontend.onrender.com)

- Full-stack hotel reservation system using MERN  
- Integrated authentication and booking workflows  
- Efficient MongoDB data modeling for users, hotels, and bookings  
- Deployed for live accessibility  

---

#### 🌿 Glow – Plant Nursery UI  
[Repository](https://github.com/Kaushik515/Glow) | 
[Live Project](https://kaushik515.github.io/Glow)

- Responsive frontend e-commerce UI  
- Interactive design using HTML, CSS, and JavaScript  
- Focused on clean layout and user experience  

---

### 🏆 Achievements

- Knight Badge on LeetCode (1000+ problems solved)
- 3-Star Coder on CodeChef
- Ranked 59th and 887th in competitive contests
- Head Coordinator at ELITE (IT Association), NIT Raipur

---

### 📫 Connect With Me

- LinkedIn: (https://www.linkedin.com/in/kaushik-kotha-ba1a60229/)
- LeetCode: (https://leetcode.com/u/Kaushik_Kotha/)
- GitHub: https://github.com/Kaushik515

---

Currently deepening expertise in scalable backend architecture, system design, and learning Spring Boot.
